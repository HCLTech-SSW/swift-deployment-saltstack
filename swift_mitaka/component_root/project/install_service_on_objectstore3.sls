...
format_xfs_cmd: 
  cmd:
    - run
    - name: 'mkfs.xfs -f "{{ pillar['objectstorage3_drive'] }}"'
    - require: 
        - pkg: xfsprog_pkg_install
...
create_mount_directory: 
  cmd: 
    - run
    - name: 'mkdir -p /srv/node/{{ pillar['objectstorage3_drive']|replace("/dev/","") }}'
    - require: 
        - cmd: format_xfs_cmd
...
update_fstab_conf_settings:
  file:
    ...
    - text: {{ pillar['objectstorage3_drive'] }} /srv/node/{{ pillar['objectstorage3_drive']|replace("/dev/","") }} xfs noatime,nodiratime,nobarrier,logbufs=8 0 2
    - require:
...
node_mount_command:
  cmd:
    - run
    - name: 'mount /srv/node/{{ pillar['objectstorage3_drive']|replace("/dev/","") }}'
    - require: 
